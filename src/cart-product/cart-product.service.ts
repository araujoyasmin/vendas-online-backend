...
export class CartProductService {
    ...
    async insertProductInCart(insertCartDTO: InsertCartDTO, cart: CartEntity): Promise<CartProductEntity>{
       await this.productService.getProductById(insertCartDTO.productId);
  ...
  async updateProductInCart(updateCartDTO: UpdateCartDTO, cart: CartEntity): Promise<CartProductEntity>{
    await this.productService.getProductById(updateCartDTO.productId);
     ...
     const cartProduct = await this.verifyProductInCart(updateCartDTO.productId, cart.id).catch(()=> undefined);
